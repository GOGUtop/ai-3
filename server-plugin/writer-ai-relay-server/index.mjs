...
const ROOT_DIR = path.basename(DATA_ROOT) === 'default-user'
  ...
  : path.join(DATA_ROOT, 'default-user', 'writer-ai-relay');
...
async function withTimeout(config, task) {
  ...
  try { return await task(controller.signal); }
    ...
    if (error?.name === 'AbortError') throw new Error('接力 API 请求超时');
    if(error?.message==='fetch failed'){
      const codes=[error.cause?.code,...(error.cause?.errors||[]).map(e=>e.code)].filter(c=>typeof c==='string'&&/^[A-Z0-9_]+$/.test(c));
      throw new Error(`模型网络连接失败${codes.length?'（'+[...new Set(codes)].join('、')+'）':''}；尚未收到模型接口的 HTTP 响应，请检查服务器网络、DNS、证书或出站代理`);
    }
    throw error;
...
export async function init(router) {
  ...
  router.post('/native-session/status',(req,res)=>{
    const record=nativeSessions.get(req.body?.key);
    if(!record||record.owner!==req.user?.profile?.handle)return res.status(404).json({ok:false,error:'接力请求状态已过期'});
    res.json({ok:true,...record.session.inspect()});
  });
